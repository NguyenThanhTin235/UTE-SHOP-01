...
const SellerMessages = ({ searchTerm }) => {
  ...
  const fetchConversations = async () => {
    try {
      ...
      const res = await axios.get('http://localhost:5000/api/chat/shop/all-conversations', {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      const myFullName = user?.full_name || 'Shop Name'; // Customer will see this
    ...
    <div className="bg-white rounded-3xl border border-slate-200 shadow-sm flex overflow-hidden" style={{ height: '700px' }}>
      ...
      <div className="w-1/3 border-r border-slate-200 flex flex-col bg-slate-50">
        <div className="p-6 border-b border-slate-200 bg-white shrink-0">
          <h2 className="text-xl font-black text-slate-900">Customer Messages</h2>
          <p className="text-sm text-slate-500 mt-1">Manage chats with customers</p>
        </div>
