...
const SellerOrderDetail = ({ orderId, onBack }) => {
    ...
    const getTimelineSteps = () => {
        const steps = [
            ...
            { id: 'confirmed', label: 'Payment Confirmed', icon: 'check', date: order.updatedAt },
            { id: 'to_ship', label: 'To Ship', icon: 'inventory_2', date: null },
            { id: 'shipped', label: 'Shipped', icon: 'local_shipping', date: null },
            { id: 'delivered', label: 'Completed', icon: 'verified', date: null }
        ];
        ...
        <div className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto bg-[#F8FAFC]">
            ...
            <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-10 sticky top-0 z-40 shrink-0">
                <div className="flex items-center gap-4">
                    <button onClick={onBack} className="w-10 h-10 flex items-center justify-center text-slate-500 hover:bg-slate-50 rounded-xl transition-all cursor-pointer">
                        <span className="material-symbols-outlined">arrow_back</span>
                    </button>
                    <div className="h-8 w-px bg-slate-200 mx-2"></div>
                    <h1 className="text-xl font-bold text-slate-900 tracking-tight">Order Details <span className="text-[#004ac6] ml-2">{order.order_code}</span></h1>
                </div>

                <div className="flex items-center gap-4">
                    <button className="px-6 py-2.5 rounded-xl font-bold text-sm border border-slate-300 text-slate-700 hover:bg-slate-50 transition-all flex items-center gap-2 cursor-pointer">
                            ...
                            {getTimelineSteps().map((step, idx) => (
                                <div key={idx} className="flex-1 flex flex-col items-center relative z-10">
                                    <div className={`size-10 rounded-full flex items-center justify-center mb-4 ${
                                        step.status === 'completed' ? 'bg-[#004ac6] text-white ring-8 ring-[#004ac6]/10' :
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 p-8 space-y-4">
                            ...
                            <div className="flex justify-between items-center text-sm">
                                ...
                                <span className="font-black text-red-500">-{formatPrice(order.coupon_discount + order.coin_discount)}</span>
                            </div>
