      ...
      <div className="flex justify-between items-center w-full px-4 md:px-10 py-4 max-w-[1280px] mx-auto gap-8">
        <div className="flex items-center gap-8">
          ...
          <nav className="hidden md:flex gap-6">
            ...
            <Link className={`text-sm font-medium ${isActive('/promotions') ? 'text-primary underline underline-offset-8 decoration-2 font-bold' : 'text-[#434655] hover:text-primary transition-colors'}`} to="/promotions">Promotions</Link>
            <Link className={`text-sm font-medium ${isActive('/blog') ? 'text-primary underline underline-offset-8 decoration-2 font-bold' : 'text-[#434655] hover:text-primary transition-colors'}`} to="/blog">Blog</Link>
            <Link className={`text-sm font-medium ${isActive('/support') ? 'text-primary underline underline-offset-8 decoration-2 font-bold' : 'text-[#434655] hover:text-primary transition-colors'}`} to="/support">Support</Link>
          </nav>
        </div>

        {!isAuthPage && (
          <div className="hidden lg:flex flex-1 max-w-md relative group">
            <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-[#434655] group-focus-within:text-primary transition-colors">search</span>
            <input
              type="text"
              placeholder="Search for academic collections..."
              className="w-full bg-[#f2f3ff] border-none rounded-full py-2.5 pl-12 pr-4 text-sm focus:ring-2 focus:ring-primary/20 transition-all outline-none text-[#131b2e]"
              value={searchTerm}
