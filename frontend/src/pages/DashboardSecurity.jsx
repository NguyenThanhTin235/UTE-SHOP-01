  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      <header className="h-20 bg-white border-b border-[#c3c6d7]/30 flex items-center px-4 md:px-10 sticky top-0 z-40 shadow-sm">
        <button onClick={() => navigate(user?.role === 'admin' ? '/admin' : user?.role === 'manager' ? '/manager' : '/seller')} className="flex items-center gap-2 text-[#434655] hover:text-[#004ac6] transition-colors font-bold cursor-pointer">
      ...
      <div className="w-full max-w-[1280px] mx-auto px-4 md:px-10 py-8 md:py-12 flex flex-col md:flex-row gap-8 items-start">
        ...
        <aside className="w-full md:w-72 flex flex-col gap-4 md:sticky md:top-24 flex-shrink-0">
          ...
          <nav className="flex flex-col gap-1 text-left">
            <Link to={user?.role === 'admin' ? '/admin/profile' : user?.role === 'manager' ? '/manager/profile' : '/seller/profile'} className="flex items-center px-4 py-3 space-x-3 text-[#434655] hover:bg-[#f7f9ff] hover:text-[#004ac6] transition-all font-medium rounded-xl">
            ...
            <Link to={user?.role === 'admin' ? '/admin/security' : user?.role === 'manager' ? '/manager/security' : '/seller/security'} className="flex items-center px-4 py-3 space-x-3 bg-[#004ac6] text-white font-bold rounded-xl shadow-lg shadow-[#004ac6]/20 transition-all">
