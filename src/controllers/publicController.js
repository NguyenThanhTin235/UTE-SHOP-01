...
exports.getCampaigns = async (req, res, next) => {
  try {
    ...
    // Parse user id if token exists in header
    let userId = null;
    ...
    let usedCouponIds = [];
    if (userId) {
      const redemptions = await CouponRedemption.find({ user_id: userId });
      usedCouponIds = redemptions.map(r => r.coupon_id.toString());
    }
    ...
    const results = await Promise.all(campaigns.map(async (camp) => {
      // Find targets (products associated with the campaign)
      const targets = await CampaignTarget.find({ campaign_id: camp._id }).limit(12);
      const productIds = targets.map(t => t.product_id);
      ...
      // Attach media and calculate averageRating & soldCount
      const productsWithMedia = await Promise.all(products.map(async (p) => {
      ...
      // Fetch active coupons for this campaign
      ...
      return {
        ...
        coupons: coupons.map(c => {
          ...
          cObj.is_used = usedCouponIds.includes(c._id.toString());
          return cObj;
        })
      };
    }));
